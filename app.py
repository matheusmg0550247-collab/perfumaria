...
import base64
import os
from PIL import Image # Importando a ferramenta de correção de imagem
import io
...
# --- 3. FUNÇÃO INTELIGENTE DE IMAGEM (CORREÇÃO AUTOMÁTICA) ---
def get_img_as_base64(file_path):
    if not os.path.exists(file_path):
        return None
    
    try:
        # 1. Abre a imagem usando PIL (ele ignora a extensão errada e descobre o formato real)
        with Image.open(file_path) as img:
            # 2. Salva a imagem como um PNG VERDADEIRO na memória RAM
            buffered = io.BytesIO()
            img.save(buffered, format="PNG")
            # 3. Transforma esse PNG novo em base64
...
with col_C:
    ...
    # Renderiza Visor + Perfume (Agora com imagem corrigida)
